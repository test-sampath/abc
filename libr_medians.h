...
double **line_intersect(double **n2,double **B,double **n3,double **C,int m,int n);
...
double **line_intersect(double **n2,double **B,double **n3,double **C, int m,int n)
{
    double **N,**n2_t,**n3_t,**p0,**p1,**p,**P1,**P2,**solution;
    p=createMat(2,1);
    N=transpose(block(n2,n3,m,n),m,m);
    n2_t=transpose(n2,m,n);
    n3_t=transpose(n3,m,n);
    p0=mat_mul(n2_t,B,n,m,n);
    p1=mat_mul(n3_t,C,n,m,n);
    p[0][0]=p0[0][0];
    p[1][0]=p1[0][0];
    solution=solve(p,N);
    return(solution);
